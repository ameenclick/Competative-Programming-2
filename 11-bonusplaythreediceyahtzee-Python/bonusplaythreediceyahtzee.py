...
# If you only have 2 matching dice, your score is 1 0 + the sum of the matching dice.
...
def playstep2(hand, dice):
	...
	if(keep == -1):
		result.append(max(l))
		result.append(dice%10)
		dice//=10
		result.append(dice%10)
		dice//=10
	else:
		result+=[keep,keep,dice%10]
		dice//=10
	keep=0
	for e in sorted(result)[::-1]:
		keep=keep*10+e
...
def bonusplaythreediceyahtzee(dice):
	...
	if(check3Dig(hand)):
		keep=hand
		score=matchScoring(keep)
	else:
		(keep, bal)=playstep2(hand,replace)
		(keep, bal)=playstep2(keep,bal)
		score=matchScoring(keep)
	return(keep,score)
